...
paint layer under `vyto/ui` (the vector `surface` calls can't do AA, clipping,
...
```sh
lib/vyto/gfx/native/build-blend2d.sh          # linux-x64
lib/vyto/gfx/native/build-blend2d.sh <triple> # other targets
```
...
For single-file distribution, `vytoc build --bundle` statically links
...
```sh
vytoc build apps/uigfx/uigfx.vt --release --bundle -o app   # one file
```
