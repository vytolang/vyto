/* vyto/intl native backing — ICU C API (icuuc + icui18n). See intl_shim.h.
 *
 ...
 * the ICU shared objects and never has to appear on vytoc's link line.
 *
 ...
 * truncated and the Vyto caller retries with a bytes(ret+1) buffer; a negative
