/* vyto/intl native backing — ICU (icuuc + icui18n).
 *
 * The boundary is strictly UTF-8: every entry takes UTF-8 `char*` in and writes
 * UTF-8 into a caller-provided buffer (`out`,`cap` = a Vyto `bytes(cap)`), and
 * ICU's own types (UChar, UErrorCode, UCollator*, …) never cross into Vyto.
 ...
 * vyto/util/fmt's bounded-buffer shim and vyto/gfx's opaque-handle model.
 */
#ifndef VYTO_INTL_SHIM_H
#define VYTO_INTL_SHIM_H
...
/* ICU's C MessageFormat entry (umsg_format) is varargs and cannot be driven
   from a tagged argument array in portable C, so vyto/intl/message.vt builds
