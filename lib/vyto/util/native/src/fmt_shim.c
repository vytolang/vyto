/* vyto/util/fmt native backing — number formatting only.

   Each function forwards a single value to snprintf. The Vyto side builds the
   ...
   Output is bounded by the caller-provided `cap` (a Vyto `bytes(cap)` buffer);
   the returned length never exceeds cap-1, and snprintf always NUL-terminates,
   so the Vyto side can `str(buf as cstring)` safely (numeric output has no
   interior NUL). */
