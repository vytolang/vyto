...
#   native/<triple>/libblend2d.so  (prebuilt shared lib — linked + shipped)
#
# Requires: git, cmake (>=3.22 after the pin below), a C++ compiler, ninja.
# Config: static-free shared build with the JIT off (self-contained, no asmjit;
# NO_JIT is also the config hardened W^X/iOS targets need).
set -e
...
cmake -S "$WORK/blend2d" -B "$WORK/build" -GNinja -DCMAKE_BUILD_TYPE=Release \
      -DBLEND2D_STATIC=FALSE -DBLEND2D_NO_JIT=TRUE >/dev/null
ninja -C "$WORK/build" blend2d

echo "[volt/gfx] installing headers + lib for $TRIPLE…"
...
cp "$WORK/build/libblend2d.so" "$HERE/$TRIPLE/libblend2d.so"

echo "[volt/gfx] done: $HERE/$TRIPLE/libblend2d.so"
