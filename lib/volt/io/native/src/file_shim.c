/* volt/io/file native backing — thin wrappers over stdio + stat.

   Every buffer transfer is bounded by a length the Volt side supplies (a
   `bytes(n)` array's capacity), and the actual byte count is returned so Volt
   never reads past what was filled. The FILE* is an opaque `rawptr` owned by a
   Volt `File` whose `deinit` calls vfile_close exactly once. */
