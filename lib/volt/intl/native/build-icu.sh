#!/bin/sh
# Vendor a private ICU for volt/intl (the hybrid provisioning path).
#
# By default volt/intl links the *system* ICU (#link "icuuc"/"icui18n" in
# intl.vt), which needs no vendored files. Run this script only when you want a
# self-contained build — `voltc build --bundle` (static ICU baked into the exe)
...
echo "[volt/intl] downloading ICU ${ICU_VER}…"
curl -fsSL "$URL" -o "$WORK/icu.tgz"
tar -xzf "$WORK/icu.tgz" -C "$WORK"               # -> $WORK/icu/source

echo "[volt/intl] configuring + building ICU (shared + static)…"
...
echo "[volt/intl] installing headers + libs for $TRIPLE…"
...
# linker's -licuXX expects (voltc ships whatever .so is in native/<triple>).
...
# ICU's C++ runtime; --bundle lists all three .a plus these. voltc reads a
...
echo "[volt/intl] done: $HERE/$TRIPLE/{libicuuc,libicui18n,libicudata}.{so,a}"
echo "[volt/intl] system #link still works; --bundle now links the vendored static ICU."
