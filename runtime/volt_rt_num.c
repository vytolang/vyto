/* volt_rt_num.c — int and float builtin-method runtime helpers.
 *
 * Amalgamated into volt_rt.c (included at its end), so every static helper and
 * host hook defined there is visible here. The `#include "volt_rt.h"` below is
