...
| `vyto/db/pgsql` | the PostgreSQL driver — **pure Vyto**, speaking the v3 wire protocol over a socket. No libpq and no GPL question. SCRAM-SHA-256 and md5 authentication, per-column binary results so a `float8` is exact, and schema introspection over `pg_catalog`. TLS through `vyto/crypto/openssl` with the full libpq `sslmode` ladder, so managed providers work — which is the one thing that has to be installed, `libssl-dev`. `examples/105_pgwire.vt` asserts all of it — 335 assertions — against a recorded transcript, with no PostgreSQL installed |
| `vyto/db/mysql` | the MySQL / MariaDB driver — **pure Vyto**, speaking the client-server protocol over a socket. No libmysqlclient, so no GPL/LGPL question about the shipped binary and nothing to provision. Prepared statements over the binary protocol (never client-side escaping), `mysql_native_password` and `caching_sha2_password` — including the full exchange MySQL 8 demands on a cache miss, which works because the driver has TLS — real `lastInsertId`, introspection over `information_schema`. `examples/106_mywire.vt` asserts it — 210 assertions — with no MySQL installed |
